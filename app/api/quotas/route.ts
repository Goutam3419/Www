...
export async function GET(req: NextRequest) {
  try {
    ...
    const quotas = await repos.quotas.getQuotas(workspaceId);
    ...
    const message = error instanceof Error ? error.message : 'Failed to fetch resource quotas';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { workspaceId = 'ws_enterprise_01', resourceType, incrementAmount = 1 } = body;
    if (!resourceType) {
    ...
    const repos = getRepositories();
    const updated = await repos.quotas.updateUsage(workspaceId, resourceType, incrementAmount);
    return NextResponse.json({
      success: true,
      mode: getActiveDatabaseMode(),
      data: updated
    ...
    const message = error instanceof Error ? error.message : 'Failed to update resource quota usage';
    return NextResponse.json({ success: false, error: message }, { status: 500 });
  }
}
